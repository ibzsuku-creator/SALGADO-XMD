...
async function kickCommand(sock, chatId, senderId, mentionedJids, message) {
    ...
    if (!isOwner) {
        ...
        if (!isBotAdmin) {
            await sock.sendMessage(chatId, { text: `Je dois être admin pour utiliser cette commande.`}, { quoted: message });
            return;
        }
    ...
    if (usersToKick.length === 0) {
        await sock.sendMessage(chatId, { 
            text: 'Mentionne la personne ou réponds à son message pour la kick !'
        }, { quoted: message });
        return;
    }
    ...
    if (isTryingToKickBot) {
        await sock.sendMessage(chatId, { 
            text: "Je ne peux pas me kick moi-même 🤖"
        }, { quoted: message });
        return;
    }
    ...
    try {
        ...
        await sock.sendMessage(chatId, { 
            text: `${usernames.join(', ')} a été éjecté(e) avec succès ! 🥷`,
            mentions: usersToKick
        });
    } catch (error) {
        console.error('Error in kick command:', error);
        await sock.sendMessage(chatId, { 
            text: "❌ Échec de l'éjection."
        });
    }
}
